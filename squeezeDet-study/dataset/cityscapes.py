# Author: Yuanbo Zheng (11346918@qq.com) 10/15/2018

"""Image data base class for cityscapes"""
...
class cityscapes(imdb):
  ...
  def _image_path_at(self, idx):
    image_path = os.path.join(self._image_path, idx+'_leftImg8bit.png')
    assert os.path.exists(image_path), \
        'Image does not exist: {}'.format(image_path)
    return image_path

  def _load_cityscapes_annotation(self):
    ...
    for index in self._image_idx:
      filename = os.path.join(self._label_path, index+'_gtFine_polygons.txt')
